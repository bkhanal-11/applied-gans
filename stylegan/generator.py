...
class StyleGANGenerator(nn.Module):
    ...
    '''
    def __init__(self, z_dim, map_hidden_dim, w_dim, channels=[512, 256, 128, 64, 32, 16, 8]):
        ...
        self.output_conv_1 = nn.Conv2d(channels[-2], 3, kernel_size=1, stride=1)
        self.output_conv_2 = nn.Conv2d(channels[-1], 3, kernel_size=1, stride=1)
    ...
    def forward(self, noise, return_intermediate=False, alpha=0.2):
        '''
        ...
        for i in range(self.num_layers):
            ...
            if i == self.num_layers - 2:
                x_small = x
                x_small_image = self.output_tanh(self.output_conv_1(x_small))
                intermediates.append(x_small_image)
            if i == self.num_layers - 1:
                x_big = x
                x_big_image = self.output_tanh(self.output_conv_2(x_big))
...
if __name__ == "__main__":
    ...
    generator = StyleGANGenerator(z_dim, map_hidden_dim, w_dim, channels)

    # Generate a random noise vector
    batch_size = 4
    noise = torch.randn(batch_size, z_dim)

    # Generate images using the generator blocks
    images = generator(noise)
    print(images.size())

    # Save the generated images
    save_image(images, 'generated_images.png', nrow=2, normalize=True)